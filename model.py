...
class LG_LeNet(nn.Module):
    ...
    def __init__(self, rep_dim=64):
        ...
        self.conv3 = nn.Conv2d(12, 8, 5, bias=False, stride=2, padding=3)
        self.bn3 = nn.BatchNorm2d(8, eps=1e-04, affine=False)
        self.fc1 = nn.Linear(8 * 7 * 7, self.rep_dim, bias=False)
...
class LG_LeNet_Decoder(nn.Module):
    ...
    def __init__(self, rep_dim=64):
        ...
        # Decoder network
        self.deconv0 = nn.ConvTranspose2d(4, 8, 5, bias=False, padding=2)
        self.bn99 = nn.BatchNorm2d(8, eps=1e-04, affine=False)
        self.deconv1 = nn.ConvTranspose2d(8, 12, 6, bias=False, padding=2)
        self.bn98 = nn.BatchNorm2d(12, eps=1e-04, affine=False)
        self.deconv2 = nn.ConvTranspose2d(12, 24, 5, bias=False, padding=2)
        self.bn97 = nn.BatchNorm2d(24, eps=1e-04, affine=False)
        self.deconv3 = nn.ConvTranspose2d(24, 5, 5, bias=False, padding=2)

    def forward(self, x):
        x = x.view(int(x.size(0)), int(self.rep_dim / 16), 4, 4)
        x = F.interpolate(F.leaky_relu(x), scale_factor=3)
        x = self.deconv0(x)
        # print(f" deconv1 {x.shape}")
        x = F.interpolate(F.leaky_relu(self.bn99(x)), scale_factor=2)
        x = self.deconv1(x)
        # print(f" deconv2 {x.shape}")
        x = F.interpolate(F.leaky_relu(self.bn98(x)), scale_factor=2)
        x = self.deconv2(x)
        x = F.interpolate(F.leaky_relu(self.bn97(x)), scale_factor=2)
        x = self.deconv3(x)
